...
require 'open-uri'
    ...
    trs.each do |tr|
      ...
      tag_head = tr.css("div.player a").text.delete("\n").strip
      tag_ary = tr.css("div.player a").attr("href").value.split("/")[-1].split("%23")
      tag_tail = tag_ary[1]
      tag = tag_head + "#" + tag_tail
      over_tag_tmp = OverTagTmp.find_by_tag(tag)
      OverTagTmp.create(tag: tag, region: region, platform: "pc") unless over_tag_tmp
    end
