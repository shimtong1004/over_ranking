class OverUserScore < ActiveRecord::Base
  ...
  def self.add_score(user_type)
    ...
    keywords = ["처치 - 평균", "결정타 - 평균", "치유 - 평균", "준 피해 - 평균", "죽음 - 평균", "단독 처치 - 평균", "근접 공격 결정타 - 평균", "임무 기여 처치 - 평균", "폭주 시간 - 평균", "임무 기여 시간 - 평균", "점수", 
                ...
                "플레이 시간", "명중률", "승률", "승리한 게임", "치른 게임", "메달 - 금"]
      ...
      play_types.each do |play_type|
        ...
        hero_names.each do |hero_name|
          ...
          hero_datas.each do |data|
            ...
            if hero_name == "모든 영웅"
             total_hash[data.keyword] = value.delete(",").to_f if data.keyword && total_hash[data.keyword] #total
              ...
              hero_hash[data.keyword] = value.delete(",").to_f
          ...
          if hero_name != "모든 영웅"
            ...
            #자가치유
            hero_hash["자가 치유"] = hero_self_healing
            self.insert_hero_data(hero_hash, hero_name, user_type, play_type)
          end
        end
        ...
        self.insert_total_data(user_type, rush_hash, play_type, "rush")
        self.insert_total_data(user_type, support_hash, play_type, "support")
      end
    end 
  end
  
  def self.insert_total_data(user_type, total_hash, play_type, hero_type=nil)
    over_user_type_id = user_type.id
    keyword_head = ""
    eliminations = 0
    deaths = 0
    ...
    if eliminations != 0 && deaths != 0
      kda = eliminations.to_f / deaths.to_f
      over_user_score = OverUserScore.where(over_user_type_id: over_user_type_id, user_type: user_type.user_type, play_type: play_type, keyword: "#{keyword_head}KDA").first
      if over_user_score
        over_user_score.update(score: kda)
      else
    ...
    if final_blows != 0 && deaths != 0
      kd = final_blows.to_f / deaths.to_f
      over_user_score = OverUserScore.where(over_user_type_id: over_user_type_id, user_type: user_type.user_type, play_type: play_type, keyword: "#{keyword_head}KD").first
      if over_user_score
        over_user_score.update(score: kd)
      else
  ...
  def self.insert_hero_data(hero_hash, hero_name, over_user_type, play_type)
    hero_hash.each do |key, value|
      over_user_score = OverUserScore.where(over_user_type_id: over_user_type.id, user_type: over_user_type.user_type, play_type: play_type, keyword: "#{hero_name}_#{key}").first
      if over_user_score
        over_user_score.update(score: value)
      else
        OverUserScore.create(over_user_type_id: over_user_type.id, user_type: over_user_type.user_type, play_type: play_type, keyword: "#{hero_name}_#{key}", score: value)
      end
    end
  end
